...
static void init(String file_name[]){
  ...
  if(line.charAt(i)==' ' || line.charAt(i)=='\t'){ 
      int numOfSpaces = (line.charAt(i) == '\t') ? 4 : 1;
	for(int j=0;j<numOfSpaces;j++){ 
       ...
       word=""; 
    ...
    }else if(delimiters.contains(line.charAt(i))){
        if(word.length()==0){
	    word+=line.charAt(i);
	    colorLine.add(getColor(word.toString()));
	    wordLine.add(new WordNode(word,spaces));
	    ...
	    word="";
	    word+=line.charAt(i);
	    wordLine.add(new WordNode (word,0));
	    colorLine.add(getColor(word.toString()));
	    word="";
	    spaces=0;
	}
...
static void save(String file_name[]){ 
     try (BufferedWriter writer = new BufferedWriter(new FileWriter(file_name[0]))) { 
        for (ArrayList<WordNode> line : text) { 
            for (WordNode word : line) { 
                writer.write(" ".repeat(word.spaces)); 
                writer.write(word.word.toString()); 
            } 
            writer.newLine(); 
        } 
    }catch (IOException e){ 
...
//	charIndex=0;
  ...
  colors.get(X).remove(wordI); 
     ...
     text.get(X).get(wordIndex).spaces=charIndex+1; 
  ...
  colors.get(X).set(wordIndex,getColor(text.get(X).get(wordIndex).word.toString())); 
      ...
      charIndex=1; 
        ...
        if (charIndex == 0 && wordIndex!=0 && !delimiters.contains((char) input)) { 
            text.get(X).get(wordIndex - 1).word.append((char) input); 
     colors.get(X).set(wordIndex-1,getColor(text.get(X).get(wordIndex - 1).word.toString())); 
            Y++; 
        }  
 //isInWord or next to word  
        else if (charIndex>=text.get(X).get(wordIndex).spaces && !delimiters.contains(text.get(X).get(wordIndex).word.charAt(0))) { 
